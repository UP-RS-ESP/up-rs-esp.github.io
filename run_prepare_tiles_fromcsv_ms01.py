...
def calc_multistep_direction_velocity_wrapper(u_ar, v_ar):
    @nb.njit(parallel=True)
        ...
        for i in nb.prange(u_ar.shape[0]):
            for j in nb.prange(u_ar.shape[1]):
                for k in nb.prange(u_ar.shape[2]):
                    magnitude[i, j, k] = np.sqrt(
                        v_ar[i, j, k] ** 2 + u_ar[i, j, k] ** 2
                    )
...
def mask_dem_aspect_direction_wrapper(
    deltadirection, u_ar, v_ar, deltadirection_threshold=45
):
    @nb.njit(parallel=True)
    def mask_dem_aspect_direction(
        deltadirection, u_ar, v_ar, deltadirection_threshold=45
    ):
...
def nre_deltadirection_wrapper(deltadirection, deltadirection_threshold=45):
    @nb.njit(parallel=True)
    def nre_deltadirection(deltadirection, deltadirection_threshold=45):
        nre_removed = np.empty((u_ar.shape[1], u_ar.shape[2]), dtype=np.float32)
        nre_removed.fill(np.nan)
        for j in nb.prange(deltadirection.shape[1]):
            for k in nb.prange(deltadirection.shape[2]):
                nre_removed[j, k] = np.count_nonzero(
                    deltadirection[:, j, k] > deltadirection_threshold
                )
        return nre_removed

    nre_removed = nre_deltadirection(deltadirection, deltadirection_threshold=45)
    return nre_removed


def calc_dem_aspect_direction_difference_wrapper(dem_aspect, direction):
    @nb.njit(parallel=True)
...
def calc_direction_variance_wrapper(u_ar, v_ar):
    @nb.njit(parallel=True)
    ...
    stack_var = calc_direction_variance(u_ar, v_ar)
    return stack_var
...
def calc_median_magnitude_direction_wrapper(u_ar, v_ar):
    @nb.njit(parallel=True)
...
def plot_mean_uv_magnitude_nreremoved_4panel(
    velocity_magnitude,
    velocity_direction_var,
    velocity_nrm,
    nre_removed,
    dem_hs,
    pngfn,
    ...
    ax[0, 1].set_title("Direction Variance")
    h = plt.colorbar(im1, ax=ax[0, 1], orientation="horizontal", shrink=0.7)
    h.set_label("variance (degree)")
    im1 = ax[0, 2].imshow(
        velocity_direction_var,
        vmin=0,
        vmax=np.nanpercentile(velocity_direction_var, 98),
        cmap="magma_r",
    )
    im1b = ax[0, 2].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[0, 2].get_xaxis().set_ticks([])
    ax[0, 2].get_yaxis().set_ticks([])
    ax[0, 2].set_title("Direction Variance")
    h = plt.colorbar(im1, ax=ax[0, 2], orientation="horizontal", shrink=0.7)
    h.set_label("variance (degree)")
    velocity_nrm2 = velocity_nrm.copy().astype("float32")
    velocity_nrm2[velocity_nrm2 == 65535] = np.nan
    im4 = ax[1, 0].imshow(
        velocity_nrm2,
    ...
    im4b = ax[1, 0].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[1, 0].get_xaxis().set_ticks([])
    ax[1, 0].get_yaxis().set_ticks([])
    ax[1, 0].set_title("NRE after aspect filter")
    h = plt.colorbar(im4, ax=ax[1, 0], orientation="horizontal", shrink=0.7)
    h.set_label("nr. of measurements")
    im5 = ax[1, 1].imshow(
        nre_removed,
        vmin=np.nanpercentile(nre_removed, 2),
        vmax=np.nanpercentile(nre_removed, 98),
        cmap="plasma",
    )
    im5b = ax[1, 1].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[1, 1].get_xaxis().set_ticks([])
    ax[1, 1].get_yaxis().set_ticks([])
    ax[1, 1].set_title("NRE removed from aspect filter")
    h3 = plt.colorbar(im5, ax=ax[1, 1], orientation="horizontal", shrink=0.7)
    h3.set_label("nre removed")
    im6 = ax[1, 2].imshow(
        nre_removed / (nre_removed + velocity_nrm2),
        vmin=0,
        vmax=1,
        cmap="hot_r",
    )
    ...
    ax[1, 2].set_title("Fraction of timesteps removed")
    h3 = plt.colorbar(im6, ax=ax[1, 2], orientation="horizontal", shrink=0.7)
    h3.set_label("fraction removed")
    fig.suptitle("%s" % (os.path.basename(pngfn).split(".")[0]))
    fig.savefig(pngfn, dpi=300)
    plt.close()


def plot_uv_magnitude_direction_4panel(
    u_ar_single,
    v_ar_single,
    v_magnitude,
    v_deltadirection,
    ...
    im0 = ax[0, 0].imshow(u_ar_single, vmin=-0.5, vmax=0.5, cmap="PiYG")
    im0b = ax[0, 0].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[0, 0].get_xaxis().set_ticks([])
    ax[0, 0].get_yaxis().set_ticks([])
    ax[0, 0].set_title("u")
    im1 = ax[0, 1].imshow(v_ar_single, vmin=-0.5, vmax=0.5, cmap="PiYG")
    im1b = ax[0, 1].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[0, 1].get_xaxis().set_ticks([])
    ax[0, 1].get_yaxis().set_ticks([])
    ax[0, 1].set_title("v")
    im4 = ax[1, 0].imshow(v_magnitude, vmin=0, vmax=1, cmap="viridis")
    im4b = ax[1, 0].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[1, 0].get_xaxis().set_ticks([])
    ax[1, 0].get_yaxis().set_ticks([])
    ax[1, 0].set_title("Velocity magnitude")
    im5 = ax[1, 1].imshow(v_deltadirection, vmin=0, vmax=45, cmap="magma_r")
    im5b = ax[1, 1].imshow(dem_hs, cmap="gray", alpha=0.5)
    ax[1, 1].get_xaxis().set_ticks([])
    ax[1, 1].get_yaxis().set_ticks([])
    ax[1, 1].set_title("Velocity deltadirection")
    h = plt.colorbar(im1, ax=ax[0, :], orientation="horizontal", shrink=0.7)
    h.set_label("displacement (m/y)")
    h2 = plt.colorbar(im4, ax=ax[1, 0], orientation="horizontal", shrink=0.7)
    h2.set_label("velocity (m/y)")
    h3 = plt.colorbar(im5, ax=ax[1, 1], orientation="horizontal", shrink=0.7)
    h3.set_label(r"$\Delta$ direction (degree)")
...
if __name__ == "__main__":
    ...
    for i in range(xtiles):
        for j in range(ytiles):
            logging.info(
                "Processing tile %02d/%02d" % (tile_counter, (xtiles * ytiles))
            )
            xoff = tile_size * i
            yoff = tile_size * j
            ...
            direction, magnitude = calc_multistep_direction_velocity_wrapper(u_ar, v_ar)
            logging.info("Calculating aspect and direction angle difference")
            deltadirection = calc_dem_aspect_direction_difference_wrapper(
                dem_aspect[yoff : yoff + ysize, xoff : xoff + xsize], direction
            )
            # cdem = dem[yoff : yoff + ysize, xoff : xoff + xsize]
            # cdem_aspect = dem_aspect[yoff : yoff + ysize, xoff : xoff + xsize]
            # cdem_hillshade = dem_hs[yoff : yoff + ysize, xoff : xoff + xsize]
            # k=0
            # if not os.path.exists('u_deltadirection'):
            #     os.mkdir('u_deltadirection')
            # pngfn = os.path.join('u_deltadirection', os.path.basename(outfile_u[0])[:-4] + '_deltadirection.png')
            # plot_uv_magnitude_direction_4panel(
            #     u_ar[k,:,:],
            #     v_ar[k,:,:],
            #     magnitude[k,:,:],
            #     deltadirection[k,:,:],
            ...
            u_ar, v_ar = mask_dem_aspect_direction_wrapper(
                deltadirection,
                u_ar,
                v_ar,
                deltadirection_threshold=deltadirection_threshold,
            )
            # pngfn = os.path.basename(outfile_u[0])[:-4] + '_deltadirection_aftermasking.png'
            # plot_uv_magnitude_direction_4panel(
            #     u_ar[k,:,:],
            #     v_ar[k,:,:],
            #     magnitude[k,:,:],
            ...
            velocity_magnitude, velocity_direction, velocity_nrm = (
                calc_median_magnitude_direction_wrapper(u_ar, v_ar)
            )
            logging.info("Calculating direction variance after aspect masking")
            velocity_direction_var = calc_direction_variance_wrapper(u_ar, v_ar)
            logging.info("Plotting mean velocity, direction, variances, nre to png")
            if not os.path.exists(dirprefix + "tile_plots"):
                os.mkdir(dirprefix + "tile_plots")
            pngfn = os.path.join(
                dirprefix + "tile_plots",
                "velocity_magnitude_tile_x%02d_y%02d.png" % (i, j),
            )
            cdem_hillshade = dem_hs[yoff : yoff + ysize, xoff : xoff + xsize]
            nre_removed = nre_deltadirection_wrapper(
                deltadirection, deltadirection_threshold
            )
            nre_removed[np.isnan(velocity_magnitude)] = np.nan
            plot_mean_uv_magnitude_nreremoved_4panel(
                velocity_magnitude,
                velocity_direction_var,
                velocity_nrm,
                nre_removed,
