...
def load_dem_aspect_slope_files(dem_fname):
    ...
    aspect_fname_lst = glob.glob(
        os.path.join(dem_dir, os.path.basename(dem_fname)[:-4] + "_aspect.tif")
    )
    ...
    slope_fname_lst = glob.glob(
        os.path.join(dem_dir, os.path.basename(dem_fname)[0:-4] + "_slope.tif")
    )
    ...
    hs_fname_lst = glob.glob(
        os.path.join(dem_dir, os.path.basename(dem_fname)[0:-4] + "_hs.tif")
    )
...
def plot_3panel_overview(
    ...
    fig, ax = plt.subplots(
        nrows=1, ncols=3, figsize=(16, 6), dpi=300, layout="constrained"
    )
    im0 = ax[0].imshow(
        dem,
    ...
    h = plt.colorbar(im2, ax=ax[1], orientation="horizontal", shrink=0.7)
