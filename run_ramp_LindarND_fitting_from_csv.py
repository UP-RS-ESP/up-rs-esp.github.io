...
def load_dem_files(dem_fname):
    ...
    return dem, dem_gt, dem_proj, dem_epsg
...
def plot_clip_u_v_dir_vel_average_loop(
    dem,
    dem_slope,
    dem_hs,
    u_ar,
    ...
    for i in tqdm.tqdm(range(u_ar.shape[0]), desc="Plotting PNGs"):
        direction_ar, ar_vel = calc_direction_velocity(u_ar[i, :, :], v_ar[i, :, :])
        direction_ar_gf, ar_gf_vel = calc_direction_velocity(
            u_ar_gf[i, :, :], v_ar_gf[i, :, :]
        )
        if not os.path.exists(outdir):
            os.mkdir(outdir)
        fname1 = os.path.basename(u_files[i]).split(".")[0][:-1]
        pngfn = os.path.join(outdir, fname1 + "6panel_vu_my.png")
        plot_single_4panel_u_v_my(
            u_ar[i, :, :],
            v_ar[i, :, :],
            u_ar_gf[i, :, :],
            v_ar_gf[i, :, :],
        ...
        pngfn = os.path.join(outdir, fname1 + "6panel_velocity_my.png")
        plot_single_4panel_vel_my(
            ar_vel,
            ar_gf_vel,
            stack_vel,
            stack_gf_vel,
...
def correct_striping_rotate(bm, pngfn, plot_figure=True):
    ...
    bm_median_ax0 = np.nanmedian(bm_r, axis=0)
    bm_p25_ax0, bm_p75_ax0 = np.nanpercentile(bm_r, [25, 75], axis=0)
    ...
    if plot_figure:
        ...
        ax["D"].plot(bm_p25_ax0, "-", color="navy", lw=0.5, label="25p")
        ax["D"].plot(bm_p75_ax0, "-", color="darkred", lw=0.5, label="75p")
        # ax["D"].plot(
...
def ArithmeticDegree_to_GeographicDegree(angle):
    """
    Convert arithmetic degree (numpy) with 0 degree pointing along the x axis (90 Degree Geographic Direction)
    to Geographic Direction. 0 Degree Geographic Direction is pointing northward (up).
    This version converts a scalar value.
    """
    if angle > 90:
        return 450 - angle
    else:
...
def calc_u_stats(outfile_u, dem_step=2, plot_pngs=False):
    ...
    for i in tqdm.tqdm(range(len(outfile_u))):
        u_file = outfile_u[i]
        deltaT, date1, date2, u, _, _ = load_u_file(u_file)
        uvar = np.nanvar(u).astype(np.float32)
        umean = np.nanmean(u).astype(np.float32)
        u10, u25, umedian, u75, u90 = np.nanpercentile(u, [10, 25, 50, 75, 90]).astype(
            np.float32
        )
        ...
        xcoord = xcoord[~np.isnan(u)]
        ycoord = ycoord[~np.isnan(u)]
        u = u[~np.isnan(u)]
        A = np.c_[
            xcoord,
            ycoord,
        ...
        U, residuals, rank, s = np.linalg.lstsq(
            A.astype(np.float32), u.astype(np.float32), rcond=None
        )
        # residuals = np.sum(p1_du**2)
        slope_deg = np.rad2deg(np.arctan(np.sqrt(U[0] ** 2 + U[1] ** 2)))
        aspect_deg = ArithmeticDegree_to_GeographicDegree(
            np.rad2deg(np.arctan2(U[1], -U[0]))
        )
        U_xy = U[0] * xcoord + U[1] * ycoord + U[2]
        p1_du = u - U_xy
        p1_du10, p1_du25, p1_dumedian, p1_du75, p1_du90 = np.nanpercentile(
            p1_du, [10, 25, 50, 75, 90]
        ).astype(np.float32)
        p1_du_perc100 = np.nanpercentile(p1_du, np.arange(0, 100, 1)).astype(np.float32)
        du_percentiles.append(p1_du_perc100)
        mse = (np.square(p1_du)).mean(axis=0)
        p1_rmse = np.sqrt(mse).astype(np.float32)
        A = np.vstack(
            (
                np.ones(len(xcoord), dtype=np.float32),
        ...
        U2, residuals2, _, _ = np.linalg.lstsq(
            A.astype(np.float32),
            u.astype(np.float32),
            rcond=None,
        )
        if len(residuals2) == 0:
            residuals2 = u - np.dot(A, U2)
            residuals2 = np.asarray([np.sum(residuals2**2).astype(np.float32)])
        fxx = U2[4]
        fyy = U2[5]
        fxy = U2[3]
        ...
        p2_du = u - U2_xy
        mse2 = (np.square(p2_du)).mean(axis=0)
        p2_rmse = np.sqrt(mse2)
        value_stats.append(
            [
                i,
                ...
                aspect_deg,
                p1_rmse,
                np.mean(p1_du).astype(np.float32),
                np.var(p1_du).astype(np.float32),
                p1_du10,
                p1_du25,
                p1_dumedian,
                p1_du75,
