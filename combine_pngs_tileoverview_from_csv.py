...
def create_u_fnames_from_csv(csv_fname, dirname):
    ...
    for i in range(len(date_pairs)):
        outfname_png = "%d_%d_os%02d_bs%02d_sr%02d_ms%02d_u_tileoverview.png" % (
            date_pairs[i, 0],
            date_pairs[i, 1],
            oversampling,
            block_size,
...
def create_v_fnames_from_csv(csv_fname, dirname):
    date_pairs = np.genfromtxt(csv_fname, delimiter=",")
    logging.info("Loading %d files" % len(date_pairs))
    logging.info("Data directory is %s" % (dirname))
    oversampling = int(os.path.basename(dirname).split("_")[1][2:])
    ...
    outfile_png = []
    for i in range(len(date_pairs)):
        outfname_png = "%d_%d_os%02d_bs%02d_sr%02d_ms%02d_v_tileoverview.png" % (
            date_pairs[i, 0],
            date_pairs[i, 1],
            oversampling,
            block_size,
...
if __name__ == "__main__":
    ...
    # Loading PNG files
    outfile_png = create_u_fnames_from_csv(csv_fname, png_dirname)
    with open("%s_u.filelist" % csv_fname, "w") as f:
        for line in outfile_png:
            f.write(f"{line}\n")
    # run magick:
    # magick @corr_dates_sd1_cc30_B_u.filelist -compress jpeg corr_dates_sd1_cc30_B.pdf
    #
    outfile_png = create_v_fnames_from_csv(csv_fname, png_dirname)
    with open("%s_v.filelist" % csv_fname, "w") as f:
        for line in outfile_png:
            f.write(f"{line}\n")
    # run magick:
    # magick @corr_dates_sd1_cc30_B_u.filelist -compress jpeg corr_dates_sd1_cc30_B.pdf
